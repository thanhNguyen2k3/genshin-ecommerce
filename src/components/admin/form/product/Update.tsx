...
import { Checkbox, Form, Input, InputNumber, Select, Image, message } from 'antd';
...
const UpdateProduct = ({ categories, regions, visions, weapons, sizes, colors, product }: Props) => {
    ...
    // Variants
    const [colorIds, setColorIds] = useState<string[]>(product?.colors.map((color) => color.colorId!)!);
    const [sizeIds, setSizeIds] = useState<string[]>(product?.sizes.map((size) => size.sizeId)!);
    const [checked, setChecked] = useState(product?.variants.length! > 0 ? true : false);
    ...
    useEffect(() => {
        form.setFieldsValue({
            ...
            saleOff: Number(product?.saleOff),
        });
    }, [form]);
    // form values end
        ...
        if (hasDupsObjects(filter) && values.variants.length > 0 && (checkSize || checkColor)) {
            ...
            try {
                ...
                const res = await instance
                    .patch(`/api/pr/product/${product?.id}`, {
                        ...values,
                        description: editors,
                ...
                setLoading(false);
                toast.error(`Cập nhật sản phẩm ${values.name} không thành công`);
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        <div>
            <Form form={form} layout="vertical" onFinish={onFinish}>
                ...
                <div className="grid gap-x-3 xl:grid-cols-3 grid-cols-1">
                    ...
                    <div className="col-span-1 space-y-3">
                        ...
                        <div className="border shadow-2xl bg-white p-3 rounded">
                            <Form.Item
                                ...
                                name={'type'}
                            >
                                <StyleSelect defaultValue={product?.type || ''}>
                                    {types.map((type) => (
                        ...
                        {/*  Characters*/}
                        <div className="px-4 pb-4 border shadow-2xl bg-white p-3 rounded">
                            ...
                            <Checkbox.Group
                                ...
                                defaultValue={product?.groupCharacter?.map((character) => character.character.id)!}
