        ...
        <div>
            <div className="max-w-full overflow-x-auto overflow-hidden">
                <table className="table-auto border border-gray-300 w-full min-w-[500px]">
                    <thead>
                        <tr>
                            <th className="py-2">Tên thương hiệu</th>
                        </tr>
                    </thead>
                    <tbody>
                        {brands.map((brand) => (
                            <tr key={brand.id}>
                                <td className="text-center border border-gray-300 w-full px-2 py-2">{brand.name}</td>
                                <td className="border border-gray-300 px-2 py-2">
                                    <StyleButton onClick={() => showModal(brand)}>Sửa</StyleButton>
                                    <StyleButton
                                        className="!bg-red-500 mt-1"
                                        onClick={() => showDeleteConfirm(brand.id)}
                                    >
                                        Xóa
                                    </StyleButton>
                                </td>
