...
import { Brand, Region } from '@prisma/client';
...
const RegionData = ({ regions }: Props) => {
    // Router
            ...
            <div className="max-w-full overflow-x-auto overflow-hidden">
                <table className="table-auto border border-gray-300 w-full min-w-[500px]">
                    ...
                    <tbody>
                        {regions.map((region) => (
                            <tr key={region.id}>
                                <td className="text-center border border-gray-300 w-full px-2 py-2">{region.name}</td>
                                <td className="border border-gray-300 px-2 py-2">
                                    <StyleButton onClick={() => showModal(region)}>Sửa</StyleButton>
                                    <StyleButton
                                        className="!bg-red-500 mt-1"
                                        onClick={() => showDeleteConfirm(region.id)}
                                    >
                                        Xóa
                                    </StyleButton>
                                </td>
